...
class Secure_Shield_Backup {
    ...
    public function create_backup() {
        if ( ! class_exists( 'ZipArchive' ) ) {
            return new WP_Error( 'missing_zip', __( 'ZipArchive is required for backups.', SECURE_SHIELD_TEXT_DOMAIN ) );
        }
        ...
        if ( true !== $zip->open( $filename, ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) {
            return new WP_Error( 'unable_to_create_backup', __( 'Unable to create backup archive.', SECURE_SHIELD_TEXT_DOMAIN ) );
        }
    ...
    public function restore_backup( $file ) {
        ...
        if ( ! file_exists( $file ) ) {
            return new WP_Error( 'missing_backup', __( 'Backup file not found.', SECURE_SHIELD_TEXT_DOMAIN ) );
        }

        if ( ! class_exists( 'ZipArchive' ) ) {
            return new WP_Error( 'missing_zip', __( 'ZipArchive is required for restore.', SECURE_SHIELD_TEXT_DOMAIN ) );
        }

        $zip = new ZipArchive();
        if ( true !== $zip->open( $file ) ) {
            return new WP_Error( 'unable_to_open', __( 'Unable to open backup archive.', SECURE_SHIELD_TEXT_DOMAIN ) );
        }
