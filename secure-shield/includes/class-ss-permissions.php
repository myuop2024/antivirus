...
class Secure_Shield_Permissions {
    ...
    public function check_permissions() {
        ...
        foreach ( $paths as $path => $recommended ) {
            ...
            if ( $perms !== $recommended ) {
                $issues[ wp_normalize_path( $path ) ] = sprintf(
                    __( 'Permission %1$s detected, recommend %2$s.', SECURE_SHIELD_TEXT_DOMAIN ),
                    $perms,
                    $recommended
                );
            }
        ...
        if ( ! empty( $issues ) ) {
            do_action( 'secure_shield/log', __( 'Permission issues detected.', SECURE_SHIELD_TEXT_DOMAIN ), 'warning' );
        }
